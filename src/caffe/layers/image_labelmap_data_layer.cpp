...
void ImageLabelmapDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  int rec_height = std::min(height, 500);
  int rec_width = std::min(width, 500);
  int start_x = std::rand() % (width - rec_width + 1);
  int start_y = std::rand() % (height - rec_height + 1);
  cv_img = cv_img(cv::Rect(start_x, start_y, rec_width, rec_height));
  cv_gt = cv_gt(cv::Rect(start_x, start_y, rec_width, rec_height));*/
